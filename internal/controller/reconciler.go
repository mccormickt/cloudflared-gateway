...
func (r *GatewayReconciler) apply(ctx context.Context, gw *gwapiv1.Gateway, gc *gwapiv1.GatewayClass) error {
	...
	tunnel, err := r.CloudflareClient.GetTunnelByName(ctx, gw.Name)
	...
	if tunnelFound && regenerated {
		...
		if err := r.CloudflareClient.DeleteTunnel(ctx, tunnel.ID); err != nil {
			return CloudflareError(err)
		}
		tunnelFound = false
	}
	...
	if !tunnelFound {
		created, err := r.CloudflareClient.CreateTunnel(ctx, gw.Name, secret)
		if err != nil {
			return CloudflareError(err)
		}
		tunnel = created
	...
	token := cfclient.BuildTunnelToken(r.CloudflareClient.AccountID(), tunnel.ID, secret)
...
func (r *GatewayReconciler) cleanup(ctx context.Context, gw *gwapiv1.Gateway) error {
	logger := log.FromContext(ctx)
	var firstErr error
...
func (r *GatewayReconciler) applyDeployment(ctx context.Context, desired *appsv1.Deployment) error {
...
func (r *GatewayReconciler) collectHTTPRoutes(ctx context.Context, gw *gwapiv1.Gateway) ([]gwapiv1.HTTPRoute, error) {
